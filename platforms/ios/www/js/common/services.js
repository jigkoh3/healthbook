angular.module('healthBook.services', [])
            ...
            if (settings.Type == 'detail') {
                ...
                for (var element in settings.returnElements) {
                    if (settings.returnElements.hasOwnProperty(element)) {
                        returnData[element] = settings.returnElements[element](originalData);
                    }
                }
                ...
                for (var i = 0; i < originalData.length; i++) {
                    ...
                    for (var element in settings.returnElements) {
                        if (settings.returnElements.hasOwnProperty(element)) {
                            returnData[i][element] = settings.returnElements[element](originalData[i]);
                        }
                    }
                }
            }
...
.factory('dataService', ['$http', function ($http) {
    ...
    return {
        ...
        getAll: function () {
            ...
            $http({
                method: 'GET',
                url: 'http://203.155.165.20/healthbookP2/api/Values'
            })
